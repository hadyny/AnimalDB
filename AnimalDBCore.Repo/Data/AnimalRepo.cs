...
namespace AnimalDBCore.Infrastructure.Data
{
    public class AnimalRepo : IAnimal, IDisposable
    {
        ...
        public AnimalRepo(AnimalDBContext db) => this.db = db;
        ...
        {
            return db.Animals.Find(animalId)?.Offspring.Where(m => !m.Child.DeathDate.HasValue).Select(m => m.Child);
        }
        ...
        {
            return db.Animals
                        .Where(m => !m.DeathDate.HasValue && EF.Functions.DateDiffDay(m.LastChecked, DateTime.Now) != 0)
                        .OrderBy(m => m.UniqueAnimalId.Length)
                        .ThenBy(m => m.UniqueAnimalId)
                        .ToList();
        }
        ...
        public IEnumerable<Animal> GetLivingAnimalsNotCheckedTodayForEmailing()
        {
            return db.Animals
                        .Where(m => 
                                ...
                                EF.Functions.DateDiffDay(m.LastChecked, DateTime.Now) != 0 && 
                                m.Room.EmailUpdates)
                        .OrderBy(m => m.UniqueAnimalId.Length)
                        .ThenBy(m => m.UniqueAnimalId)
                        .ToList();
        ...
        public IEnumerable<Animal> GetLivingAnimalsNotFedTodayForEmailing()
        {
            return db.Animals
                        .Where(m => !m.DeathDate.HasValue && 
                                    ...
                                    EF.Functions.DateDiffDay(m.Feeds.OrderByDescending(n => n.Date).FirstOrDefault().Date, DateTime.Now) != 0) &&
                                    m.Room.EmailUpdates)
                        .OrderBy(m => m.UniqueAnimalId.Length)
                        .ThenBy(m => m.UniqueAnimalId)
                        .ToList();
